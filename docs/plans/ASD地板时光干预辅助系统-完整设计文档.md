...
8. [LangGraph工作流编排](#8-langgraph工作流编�?
9. [技术栈与实现](#9-技术栈与实�?
10. [前端架构方案](#10-前端架构方案)
11. [部署架构](#11-部署架构)
12. [实施计划](#12-实施计划)
...
- **社交沟通困�?*：叫名字没反应、语言发育迟缓、不会使用手势、难以与同龄人互�?
- **重复受限行为**：刻板动作、维持固定流程、强烈兴趣偏好、感官反应异�?
...
- **六大情绪发展里程�?*：自我调节、亲密关系、双向沟通、复杂沟通、情绪想法、逻辑思�?
...
- **捕捉微观变化**：不只记录量表分数，而是敏锐捕捉每个孩子的微小成长细�?
- **动态时序追�?*：建立完整时间线，可视化展示进步轨迹
- **个性化干预**：每个孩子都是独特的个体，系统动态适配其特�?
- **专业赋能家长**：让普通家长也能像专业治疗师一样精准干�?
...
1. **闭环驱动**：每次游戏都会更新孩子画像，影响下一次推�?
2. **人机协作**：AI负责分析和建议，家长负责执行和反�?
...
#### 痛点1：专业知识匮�?
- **问题**：家长不懂儿童神经发育、发展心理学，对"跟随兴趣""打开-关闭沟通圈"等原则只停留在字面理�?
- **后果**：要么完全放任不管，要么用成人想法强行干预，违背地板时光核心逻辑

#### 痛点2：不知道孩子对什么感兴趣
- **问题**：孩子对常规玩具"没反�?，兴趣表现隐蔽、重复、感官导向（盯着风扇、转车轮、摸材质�?
- **后果**：家长能观察到行为，但不知道如何描述，看不到规律和趋�?

#### 痛点3：培训费用高�?
- **问题**：专业培训动辄几千到几万元，普通家庭难以承�?
- **后果**：只能东拼西凑网上碎片化知识，干预缺乏系统性和专业�?

#### 痛点4：医家协同断�?
- **问题**：医生培训内容通用化，与孩子实际情况脱节；家长缺乏专业表达能力，无法精准反馈孩子表�?
...
| 特�?| 实现方式 | 价�?|
|------|---------|------|
| **动态State** | 灵活的时序数据结�?| 捕捉微观变化，支持个性化维度扩展 |
| **LangGraph工作�?* | 状态驱动的Agent协同 | 信息连贯，支持中断恢�?|
| **Graphiti记忆** | 时序图数据库 | 自动识别里程碑，分析因果关联 |
| **人机协作** | 关键节点暂停等待家长反馈 | AI不会"自说自话"，确保准确�?|
| **多模态融�?* | 视频+语音+文字 | 全方位观察孩子，提升分析准确�?|
...
传统State�?*固定的字�?*（如`name`, `age`, `score`），而本系统的State�?*活的时序数据**�?
...
interface DynamicInterventionState {
  ...
  childTimeline: {
    ...
    metrics: {
      ...
      customDimensions: {
        ...
        // spontaneousSmiles, verbalInitiation�?
      };
    };
  ...
  // 当前工作上下�?
  currentContext: {
    recentTrends: object;  // AI识别的最新趋�?
    attentionPoints: string[];  // 值得关注的亮�?
    activeGoals: object[];  // 当前聚焦的目�?
  };
...
|-------|------|-------------|------|------|
| **初评估Agent** | 建立孩子画像 | 第一�?| 医院报告/量表 | 多维度观察框�?|
| **推荐Agent** | 智能游戏推荐 | 第二�?| 孩子画像+最新观�?| 个性化游戏方案 |
| **实时指引Agent** | 游戏过程指导 | 第三�?| 游戏步骤+实时观察 | 语音提示+引导 |
| **观察捕获Agent** | 记录孩子表现 | 第三步（并行�?| 快捷按钮+语音+视频 | 微观观察事件 |
| **视频分析Agent** | AI视频分析 | 第四�?| 录制视频 | 行为+情绪分析 |
| **总结Agent** | 单次干预汇�?| 第四�?| 所有游戏数�?| 成长快照 |
| **反馈表Agent** | 智能问卷生成 | 第五�?| 干预总结 | 个性化反馈�?|
| **记忆更新Agent** | Graphiti融合 | 第六�?| 新观�?反馈 | 更新后的记忆网络 |
| **再评估Agent** | 更新孩子画像 | 第六�?| 全部历史数据 | 进展报告+下一步计�?|
...
```typescript
{
  childTimeline: {
    ...
    metrics: {
      ...
      customDimensions: {
        sensorySensitivity: MetricTimeSeries,  // 根据家长描述动态创�?
...
2. **语音记录**（最推荐�?
```
家长说："今天玩积木的时候，辰辰突然抬头看了我一眼，大概�?秒钟"
...
```typescript
{
  videoAnalysis: {
    duration: "15:23",
    highlights: [
      {time: "02:15", event: "主动眼神接触", duration: "3�?},
      {time: "08:42", event: "微笑回应", confidence: 0.92},
      {time: "12:30", event: "主动递积�?, significance: "breakthrough"}
    ],
    metrics: {
      eyeContactCount: 5,
      smileCount: 8,
...
**闭环触发**�?
- 如果某个维度需要调�?�?返回推荐Agent，推荐新游戏
- 如果整体良好 �?继续当前计划

#### Agent 10：可视化Agent（辅助功�?�?

**核心职责**：将孩子的成长数据可视化呈现

**可视化类�?*�?
1. **多维度雷达图**：展示六大情绪发展里程碑的当前水�?
2. **时间�?*：标注重要里程碑和突破性事�?
3. **趋势�?*：展示各项指标的变化曲线
4. **热力�?*：展示微观进步的密集程度
5. **因果网络�?*：展示干预与效果的关�?
...
// AI输出�?
{
  milestoneDetected: {
    ...
    comparison: "此前30天只有被动眼神接�?,
    celebrationMessage: "🌟 这是孩子的第一次主动眼神接触！这是情感联结的重要突�?
  }
}
```
...
```javascript
{
  ...
  insight: "微笑频率每周稳定增长，孩子的情绪表达能力正在快速提�?
}
```
...
```javascript
{
  correlation: {
    ...
    then: "孩子更容易出现主动互�?,
    confidence: 0.82,
    insight: "孩子在上午精力充沛时，且游戏时长不超�?5分钟时，互动意愿更强",
    suggestion: "可以尝试将重点游戏安排在上午，控制在15分钟�?
  }
}
```
...
整个干预流程是一�?*LangGraph有向循环�?*，严格对应核心业务流程：
...
workflow.addConditionalEdges(
  ...
  {
    recommend_new: 'recommend_games',  // 返回第二�?
    continue_current: 'generate_visualization'
  }
);
```

#### 特�?：Checkpoint机制（支持中断恢复）
...
```
1. 初评�?�?建立childTimeline（metrics + microObservations�?
2. 推荐 �?currentContext.selectedGames
3. 游戏开�?�?currentSession初始�?
...
7. 生成反馈�?�?currentSession.feedbackForm + waitForUser=true
8. [暂停] �?家长填写反馈
9. 恢复 �?记忆更新 �?childTimeline新增dataPoints + analysis更新
10. 再评�?�?currentContext.reEvaluation + nextSteps
...
POST   /api/auth/verify_login     // 验证码登�?
...
// ===== 干预会话（第三步�?====
POST   /api/sessions              // 创建新会�?
PUT    /api/sessions/:id/start    // 开始会�?
...
POST   /api/observations/guided   // 引导式记录会�?

// ===== 视频处理（第四步�?====
POST   /api/videos/upload         // 上传游戏视频
GET    /api/videos/:id/status     // 查询分析状�?

// ===== 反馈表（第五步）=====
POST   /api/feedback/:sessionId/submit   // 提交反馈

// ===== 观察分析（辅助功�?�?====
GET    /api/observations/analysis/latest // 获取最新分�?
GET    /api/observations/milestones      // 获取里程碑列�?
...
React 18 + TypeScript
  �?
Ant Design Mobile (移动端优�?
  �?
Zustand (本地状�? + React Query (服务端状�?
  �?
Vite (快速构�?
```
...
export const useVideoRecorder = () => {
  ...
  const startRecording = async () => {
    const stream = await navigator.mediaDevices.getUserMedia({
      video: { facingMode: 'environment' },  // 后置摄像�?
      audio: true
    });

    const mediaRecorder = new MediaRecorder(stream);
    mediaRecorder.start(1000);  // 每秒一个切�?
    setIsRecording(true);
  };
...
| 需求确�?| �?�?| 需求文�?| 痛点明确、方案可�?|
| 架构设计 | �?�?| 设计文档 | 本文�?|
| 环境搭建 | �?�?| 开发环�?| LangGraph工作流跑�?|
| Agent开�?| �?-8�?| 10个Agent | 单元测试通过 |
| 前端开�?| �?-10�?| Web应用 | 核心页面可用 |
| 联调测试 | �?1�?| 测试报告 | 主要bug修复 |
| MVP发布 | �?2�?| 可演示系�?| 家长可完整使�?|
...
1. **以核心流程为主线**
   - 系统围绕"评估→推荐→实施→总结→反馈→再评�?的闭环设�?
   - 所有功能都服务于这个核心流�?

2. **家长是主角，AI是助�?*
   - 家长建立情感联结，AI负责记录和分�?
   - AI不会"自说自话"下结论，而是呈现证据让家长判�?
...
本文档详细描述了ASD地板时光干预辅助系统的完整设计方案，从核心业务流程出发，涵盖了痛点分析、系统架构、Agent设计、微观变化捕捉、工作流编排、技术实现、前端设计、部署方案和实施计划。系统的核心理念�?*围绕"评估→推荐→实施→总结→反馈→再评�?的闭环，通过AI多Agent协同，让家长能够像专业治疗师一样精准干预，同时敏锐捕捉每个孩子的微小成长细�?*�?
