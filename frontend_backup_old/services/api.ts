...
export const ApiService = {
  ...
  async startSession(childId: string, gameId: string): Promise<any> {
    const res = await fetch(`${API_BASE_URL}/api/game/session/start`, {
      method: 'POST',
  ...
  async addObservation(sessionId: string, content: string, behavior?: string, feeling?: string): Promise<any> {
    const res = await fetch(`${API_BASE_URL}/api/game/session/${sessionId}/observation`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ content, child_behavior: behavior, parent_feeling: feeling })
    });
    if (!res.ok) throw new Error(await res.text());
    return await res.json();
  },

  async endSession(sessionId: string, data: { engagement?: number, achievement?: number, satisfaction?: number, notes?: string, videoFile?: File }): Promise<any> {
    const formData = new FormData();
    formData.append('session_id', sessionId);
    if (data.engagement !== undefined) formData.append('child_engagement_score', data.engagement.toString());
    if (data.achievement !== undefined) formData.append('goal_achievement_score', data.achievement.toString());
    if (data.satisfaction !== undefined) formData.append('parent_satisfaction_score', data.satisfaction.toString());
    if (data.notes) formData.append('notes', data.notes);
    if (data.videoFile) formData.append('video', data.videoFile);

    const res = await fetch(`${API_BASE_URL}/api/game/session/end`, {
      method: 'POST',
      body: formData
    });
    if (!res.ok) throw new Error(await res.text());
  ...
  async summarizeSession(sessionId: string): Promise<any> {
    const res = await fetch(`${API_BASE_URL}/api/game/summarize`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
