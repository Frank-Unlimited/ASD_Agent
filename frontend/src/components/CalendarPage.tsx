...
export const PageCalendar = ({ navigateTo, onStartGame }: {
  ...
  onStartGame: (gameId: string, directPlay?: boolean, sourcePage?: Page) => void
}) => {
      ...
      <div className="h-full flex flex-col bg-gradient-to-br from-blue-50 via-white to-purple-50">
        ...
        >
                ...
                return (
                  <div key={i} className="flex flex-col items-center">
                    <button
                      ...
                      className={`w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold transition-all transform ${isSelected
          ...
          {dragOffset !== 0 && !isSwipeAnimating && Math.abs(dragOffset) > 20 && (
            <div className={`absolute top-1/2 transform -translate-y-1/2 transition-all ${dragOffset < 0 ? 'right-4' : 'left-4'
              }`}>
              <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${dragOffset < 0
                ? 'from-blue-400 to-purple-500'
                : 'from-purple-400 to-blue-500'
                } flex items-center justify-center shadow-lg animate-pulse`}>
                <ChevronRight className={`w-6 h-6 text-white ${dragOffset > 0 ? 'rotate-180' : ''
                  }`} />
              </div>
            </div>
          )}
              ...
              {monthDates.map((date, i) => {
                ...
                return (
                  <button
                    ...
                    className={`aspect-square rounded-lg flex flex-col items-center justify-center text-xs transition-all transform ${isSelected
                  ...
                  >
                    ...
                    {hasEvents && (
                      <div className={`w-1 h-1 rounded-full mt-0.5 ${isSelected ? 'bg-white' : 'bg-green-500 shadow-sm'
                        }`}></div>
                    )}
                  </button>
                );
                      ...
                      hourEvents.map(event => {
                        if (event.type === 'game') {
                          ...
                          return (
                            <div
                              key={event.id}
                              onClick={() => onStartGame(game.id, false, Page.CALENDAR)}
                        ...
                        <div key={index} className="bg-white p-3 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition">
                          ...
                          <div className="mb-3">
                            <div className="flex justify-between items-center mb-1.5">
                              ...
                              <span className={`text-xs font-black flex items-center px-2 py-0.5 rounded-lg ${match.intensity > 0
                            ...
                            <div className="relative w-full h-2 bg-gray-100 rounded-full overflow-hidden shadow-inner">
                              ...
                              <div
                                className={`absolute h-full rounded-full transition-all shadow-sm ${match.intensity > 0
