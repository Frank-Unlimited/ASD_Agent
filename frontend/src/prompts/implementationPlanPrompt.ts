...
3. **详细步骤**（5-8个步骤）：
   - 每个步骤包含：步骤标题、详细指令
   - 步骤标题：如"第一步：准备材料"、"第二步：观察并跟随孩子"
   - 详细指令：家长应该如何跟随孩子、如何回应孩子的行为（2-3句话）
   - 步骤要循序渐进，从简单到复杂
   - 考虑 ${childProfile.name} 的具体情况进行个性化调整
   - 重要：地板游戏是以孩子为主导的游戏，家长的角色是跟随和回应，而不是控制和引导
   - 不要设定每一步的预期效果，因为孩子的反应是自然发生的
...
5. steps（游戏步骤）：5-8个详细步骤，每个步骤包含：
   ...
   - instruction：详细指令（家长应该如何跟随孩子、如何回应孩子的行为，2-3句话）
...
- 每个指令要包含具体的动作、语言、时机
- 步骤要循序渐进，从简单到复杂
- 强调家长跟随孩子的主导，而不是预设每一步的效果
- 地板游戏是以孩子为中心的，家长观察并回应孩子的兴趣和节奏
`;
}
